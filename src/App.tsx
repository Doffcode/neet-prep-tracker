...
function App() {
  ...
  useEffect(() => {
    if (window.innerWidth <= 768) {
      if (activeTab === 'tracker' && dayDetailRef.current) {
        setTimeout(() => {
  ...
  return (
    <div className="container">
      <div ref={topRef} className="sticky-top">
        <header className="app-header">
          <h1>NEET Prep Tracker</h1>
          <p>Maximize marks per hour. Prioritize high-ROI chapters.</p>
        </header>
                  ...
                  <div key={subject} className="graph-row">
                    ...
                    <div className="graph-bar-container">
                      {(['S', 'A', 'B'] as Tier[]).map(tier => {
                        const bd = subjectStats[subject].tierBreakdown[tier];
                        if (bd.total === 0) return null;
                        const segWidth = getTierTotalWidth(tier, subject);
                        const fillPct = bd.total > 0 ? (bd.completed / bd.total) * 100 : 0;
                        return (
                          <div
                            key={tier}
                          ...
                          >
                            <div
                              ...
                              style={{ width: `${fillPct}%`, background: fillPct > 0 ? undefined : 'transparent' }}
                            />
                          </div>
                        );
                      })}
